...
DATA_PATH ="./dataset/"
IMAGE_DATA = ['mnist', 'cifar10', 'cifar100', 'cifar20', 'utkface', 'facescrub', 'places365', 'fashion_mnist']
    ...
    if args.dataset in IMAGE_DATA:
        if len(dst) == 2: # IMAGE_DATA without attribute
            if args.k == 2:
                ...
                segment_height = int(32 / args.k)
                if index == args.k - 1:
                    # The last party gets the last segment and the labels
                    return (dst[0][:, :, (args.k - 1)*segment_height:, :], dst[1])
                else:
                    # Passive parties get their respective segments without labels
                    return (dst[0][:, :, index*segment_height:(index+1)*segment_height, :], None)
                
            elif args.k == 1: # Centralized Training
                return (dst[0], dst[1])
            else:
                assert (args.k == 2 or args.k == 4 or args.k == 8), "total number of parties not supported for data partitioning"
                return None
...
def load_dataset_per_party(args, index):
    ...
    if args.dataset == "cifar100":
        ...
        half_dim = 14
        train_dst = datasets.FashionMNIST("~/.torch", download=True, train=True, transform=transform_fn)
        data, label = fetch_data_and_label(train_dst, args.num_classes)
        # train_dst = SimpleDataset(data, label)
        if args.need_auxiliary == 1:
            data, X_aux, label, y_aux = train_test_split(data, label, test_size=0.1, random_state=args.current_seed)
        ...
        test_dst = datasets.FashionMNIST("~/.torch", download=True, train=False, transform=transform_fn)
        data, label = fetch_data_and_label(test_dst, args.num_classes)
        # test_dst = SimpleDataset(data, label)
        test_dst = (data, label)
