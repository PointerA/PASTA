...
class ActiveParty(Party):
    def __init__(self, args, index):
        ...
        self.message_queues = [queue.Queue() for i in range(args.k-1)]
        self.message_caches = [{} for i in range(args.k-1)]
        self.Ck = None
        ...
        self.each_party_local_training_time = [0 for i in range(args.k)]
        self.each_party_communication_time = [0 for i in range(args.k)]
        self.each_party_communication_power = [] 
        if args.k == 2:
            self.each_party_communication_power = [50, 5]
        elif args.k == 4:
            self.each_party_communication_power = [100, 50, 5]
        elif args.k == 8:
            self.each_party_communication_power = [100, 100, 50, 50, 50, 5, 5]
    ...
    def check_ready(self):
        for ik in range(self.args.k-1):
            if self.finish_batch+1 in self.message_caches[ik]:
                continue
            if self.message_queues[ik].empty():
                return False
            else:
                continue
        return True
    
    def generate_Control_signal(self, Control_signal):
        for ik in range(self.args.k-1):
            if self.message_queues[ik].qsize() + len(self.message_caches[ik]) > 1:
                Control_signal[ik] = -1
            elif self.message_queues[ik].qsize() + len(self.message_caches[ik]) < 1:
                Control_signal[ik] = 1
    ...
    def give_gradient(self):
        # Ck = [0 for i in range(self.args.k-1)]
        # for ik in range(self.args.k-1):
        #     if len(self.message_caches[ik]) + self.message_queues[ik].qsize() > 1:
        #         Ck[ik] = -1
