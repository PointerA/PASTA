...
from utils.processing_decline import api4VFLAIR, api4FlexVFL
...
'avazu':0.83,'criteo':0.74,'nursery':0.99,'credit':0.82, 'fashion_mnist':0.6}  # add more about stopping accuracy for different datasets when calculating the #communication-rounds needed


class MainTaskVFL_ours(object):
    ...
    def __init__(self, args):
        ...
        # ours related
        self.weight_cache = [deque() for _ in range(args.k)]
        self.cache_size = args.cache_size
        #BN
        self.batch = [0 for _ in range(args.k)]
        self.total_batch = -1
    ...
    #ours
    def get_batch_by_index_updateBN(self, index, ik):
        i = 0
        data_loader_list = self.parties[ik].train_loader
        for parties_data in data_loader_list:
            i += 1
            if index % self.total_batch == i and index / self.total_batch < self.epochs:
                with torch.no_grad():
                    _local_pred = self.parties[ik].local_model(parties_data[0])
        
    def pred_transmit(self): # Active party gets pred from passive parties
        for ik in range(self.k):
            if self.args.communication_protocol in ['ours']:
                _model = self.weight_cache[ik][0]
                pred, pred_detach = self.parties[ik].give_pred(_model)
            else:
    ...
    def train_batch(self, parties_data, batch_label):
        ...
        if self.args.communication_protocol in ['Vanilla','FedBCD_p','Quantization','Topk'] or self.Q ==1 : # parallel FedBCD & noBCD situation
            ...
            # update parameters for all parties
            for ik in range(self.k):
                _model = self.weight_cache[ik].popleft()
                self.parties[ik].local_backward(model=_model)
            self.parties[self.k-1].global_backward()
            
            # for passive party, do local update without info exchange
            for ik in range(self.k-1):
                for q in range(self.Q[ik][self.num_total_comms]):
                    _pred, _pred_clone= self.parties[ik].give_pred() 
                    self.parties[ik].local_backward() 
            # for active party, do local update without info exchange
            for q in range(self.Q[self.k-1][self.num_total_comms]):
                _pred, _pred_clone = self.parties[self.k-1].give_pred() 
                _gradient = self.parties[self.k-1].give_gradient()
                self.parties[self.k-1].global_backward()
                self.parties[self.k-1].local_backward()
            ...
            # update chache
            for ik in range(self.k-1):
                if(self.cache_size[ik][self.num_total_comms+1] >= self.cache_size[ik][self.num_total_comms]):
                    self.weight_cache[ik].append(copy.deepcopy(self.parties[ik].local_model))
                    self.get_batch_by_index_updateBN(self.batch[ik], ik)
                    self.batch[ik] += 1
                if(self.cache_size[ik][self.num_total_comms+1] > self.cache_size[ik][self.num_total_comms]):
                    self.weight_cache[ik].append(copy.deepcopy(self.parties[ik].local_model))
                    self.get_batch_by_index_updateBN(self.batch[ik], ik)
                    self.batch[ik] += 1
            self.weight_cache[self.k-1].append(copy.deepcopy(self.parties[self.k-1].local_model))
            self.get_batch_by_index_updateBN(self.batch[self.k - 1], self.k - 1)        
            self.batch[self.k - 1] += 1
            ...
            # for passive party, do local update without info exchange
            for ik in range(self.k-1):
                for q in range(self.Q[ik]):
                    _pred, _pred_clone= self.parties[ik].give_pred() 
                    self.parties[ik].local_backward() 
            # for active party, do local update without info exchange
            for q in range(self.Q[self.k-1]):
                _pred, _pred_clone = self.parties[self.k-1].give_pred() 
                _gradient = self.parties[self.k-1].give_gradient()
                self.parties[self.k-1].global_backward()
                self.parties[self.k-1].local_backward()
    ...
    def train(self):
        ...
        #ours
        if self.args.communication_protocol in ['ours']:
            print(len(self.parties[0].train_loader))
            self.total_batch = len(self.parties[0].train_loader)
            self.Q, self.cache_size = api4VFLAIR(self.epochs, self.total_batch, self.k, self.cache_size)
            for ik in range(self.k - 1):
                for _ in range(self.cache_size[ik][0] + 1):
                    self.weight_cache[ik].append(copy.deepcopy(self.parties[ik].local_model))
                    self.get_batch_by_index_updateBN(self.batch[ik], ik)
                    self.batch[ik] += 1
            self.weight_cache[self.k - 1].append(copy.deepcopy(self.parties[self.k - 1].local_model))
        ...
        if self.args.communication_protocol in ['FlexVFL']:
            self.Q = api4FlexVFL(self.epochs, self.total_batch, self.k, self.Q)
        ...
        for i_epoch in range(self.epochs):
            ...
            if (i + 1) % print_every == 0:
                ...
                with torch.no_grad():
                    ...
                    postfix['train_loss'] = train_loss_i_epoch / (i+1)
                    postfix['train_acc'] = '{:.4f}%'.format(train_acc_i_epoch / (i+1) * 100)
                    postfix['test_acc'] = '{:.4f}%'.format(self.test_acc * 100)
                    postfix['test_auc'] = '{:.4f}%'.format(self.test_auc * 100)
                    if self.noise_test_acc != None:
                        postfix['noisy_sample_acc'] = '{:2f}%'.format(self.noise_test_acc * 100)
                    # tqdm_train.set_postfix(postfix)
                    print('Epoch {}% \t train_loss:{:.4f} train_acc:{:.4f} test_acc:{:.4f} test_auc:{:.4f}'.format(
                        i_epoch, train_loss_i_epoch / (i+1), train_acc_i_epoch / (i+1), self.test_acc, self.test_auc))
                    if self.noise_test_acc != None:
                        print('noisy_sample_acc:{:.4f}'.format(self.noise_test_acc))
        ...
        for ik in range(self.k):
            print([a / b if b != 0 else 0 for a, b in zip(self.parties[ik].pipeline_time, self.parties[ik].pipeline_counts)])
    ...
    def train_graph(self):
        ...
        for i_epoch in range(self.epochs):
            ...
            with torch.no_grad():
                ...
                postfix['train_acc'] = '{:.4f}%'.format(self.train_acc * 100)
                postfix['test_acc'] = '{:.4f}%'.format(self.test_acc * 100)
                # tqdm_train.set_postfix(postfix)
                print('Epoch {}% \t train_loss:{:.4f} train_acc:{:.4f} test_acc:{:.4f}'.format(
