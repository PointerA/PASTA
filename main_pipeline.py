...
from evaluates.MainTaskVFL_ours import *
...
def evaluate_no_attack(args):
    ...
    # Save record 
    exp_result = f"K|bs|LR|num_class|Q|top_trainable|epoch|communication|attack_name|{args.attack_param_name}|main_task_acc|{attack_metric_name},%d|%d|%lf|%d|%d|%d|%d|{args.communication_protocol}|{args.attack_name}|{args.attack_param}|{main_acc}|{attack_metric}" %\
    ...
    print(f"==stopping_iter:{stopping_iter}==stopping_time:{stopping_time}==stopping_commu_cost:{stopping_commu_cost}")
...
if __name__ == '__main__':
    ...
    # for seed in [97]: # test 5 times 
        ...
        filename = f'{args.defense_name}_{args.defense_param},model={args.model_list[str(0)]["type"]},protocol={args.communication_protocol}_{args.cache_size}.txt'
